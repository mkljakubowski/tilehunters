...
import { getActivities } from '../services/strava.js';
import { refreshToken } from '../services/strava.js';
...
router.get('/:id/polyline', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT summary_polyline FROM activities WHERE id = $1 AND user_id = $2',
    ...
    const encodedPolyline = result.rows[0].summary_polyline;
    if (!encodedPolyline) {
      return res.json([]);
    }
